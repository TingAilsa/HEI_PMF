...
csn_pmf_base = read.csv("EPA_MDL_CSN_Cluster6_F7.csv")
colnames(csn_pmf_base)[1] = "Date"
factor.number = ncol(csn_pmf_base) - 1
...
csn_conc = read.csv("/Users/ztttttt/Dropbox/HEI_PMF_files_Ting/CSN_PMF_Try_data/EPA MDL/CSN_Cluster6_2011-14_PMF_conc_EPAmdl.csv")
csn_pmf_base$Date = csn_conc$Date
csn_pmf_base$Date = as.Date(csn_pmf_base$Date, format = "%m/%d/%y")
...
csn_pmf_base_site3 = subset(csn_pmf_base,
                            SiteCode == site_row$SiteCode[3])
csn_pmf_base_site3_lm = lm(P25 ~ Factor.1 + Factor.2 + Factor.3 + Factor.4 + 
                             Factor.5 + Factor.6 + Factor.7, 
                           data = csn_pmf_base_site3)
csn_pmf_base_site3_lm_beta = summary(csn_pmf_base_site3_lm)$coefficients[, 1]
csn_pmf_base_site3_lm_beta = data.frame(csn_pmf_base_site3_lm_beta[2:length(csn_pmf_base_site3_lm_beta)])
colnames(csn_pmf_base_site3_lm_beta) = "lm.beta.site-ClusterBased"
csn_pmf_base_site3_lm_beta$Factor = rownames(csn_pmf_base_site3_lm_beta)
csn_pmf_base_site3_lm_beta$Factor.contribution = (csn_pmf_base_site3_lm_beta$lm.beta.site/
                                                    sum(csn_pmf_base_site3_lm_beta$lm.beta.site))*100

write.csv(csn_pmf_base_site3_lm_beta, "CSN_Cluster6_2011-14_EPAmdl_PMF_F7_site3.csv")
