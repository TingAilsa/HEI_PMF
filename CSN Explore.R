...
csn_sites = select(csn_data, State, SiteCode, SiteName, Longitude, Latitude, Elevation, EPACode, CountyFIPS)
csn_sites = csn_sites[!duplicated(csn_sites), ] 
write.csv(csn_sites, "CSN 156 sample sites info 2010-20.csv")

csn_data$SiteName = csn_data$CountyFIPS = csn_data$EPACode = NULL

#### detect the distribution trend of MDL for a given component with a specific region or time ####
# From previous observation of the whole dataset, we found MDL varies between sampling sites & across study period, now we try to detect if there is an trend
