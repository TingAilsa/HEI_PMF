...
# 1. Make the copy of one iniparams.txt file
cp iniparams_base.txt ini_C_${Cluster_number}_F_${Factor_number}_base.txt

# 2. Analyze the output .txt file, generate the new value for numoldsol, and replace it in other iniparams.txt series usi$
Rscript /projects/HAQ_LAB/tzhang/pmf_no_gui/file_try/slurm_txt_edit.R ini_C_${Cluster_number}_F_${Factor_number}_base.txt